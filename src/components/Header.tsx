...
export const Header: React.FC<HeaderProps> = ({
  ...
  return (
    <header className="print:hidden sticky top-0 z-40 bg-white/95 dark:bg-[#1A1D21]/95 backdrop-blur-md text-slate-800 dark:text-white border-b border-slate-200 dark:border-slate-800 shadow-sm transition-colors duration-200">
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        <div className="flex items-center justify-between h-16">
          ...
          <div className="flex items-center space-x-2 sm:space-x-2.5">
            ...
            <motion.button
              id="btn-color-theme-preset"
              type="button"
              onClick={() => setShowThemePresetModal(true)}
              whileHover={{ scale: 1.05, y: -1 }}
              whileTap={{ scale: 0.95 }}
              className="flex items-center space-x-1.5 px-2.5 py-1.5 bg-slate-100 dark:bg-slate-800/80 hover:bg-slate-200 dark:hover:bg-slate-700/80 border border-slate-300 dark:border-slate-700 text-slate-700 dark:text-slate-200 rounded-xl text-xs font-semibold shadow-xs transition-all cursor-pointer group"
              title={`Palet Warna: ${activeColorPreset.name} — Klik untuk kustomisasi warna aksen dashboard`}
              aria-label={`Palet Warna: ${activeColorPreset.name}`}
            >
              <div className="relative flex items-center shrink-0 w-5 h-4 pointer-events-none">
                <span
                  className="w-3.5 h-3.5 rounded-full shadow-xs border border-white dark:border-slate-800 z-10 group-hover:scale-110 transition-transform"
                  style={{ backgroundColor: activeColorPreset.primaryHex }}
                />
                <span
                  className="w-3 h-3 rounded-full shadow-xs border border-white dark:border-slate-800 -ml-1.5 opacity-90 group-hover:scale-110 transition-transform"
                  style={{ backgroundColor: activeColorPreset.secondaryHex }}
                />
              </div>
              <span className="hidden xl:inline text-[11px] font-bold text-slate-800 dark:text-slate-200">{activeColorPreset.name}</span>
              <Palette className="w-3.5 h-3.5 text-slate-400 dark:text-slate-500 group-hover:text-indigo-500 dark:group-hover:text-indigo-400 transition-colors" />
            </motion.button>
                ...
                <AnimatePresence>
                  {showProfileMenu && (
                    ...
                    <div className="py-1">
                      ...
                      {/* User Settings & Mascot Bubble Preferences */}
                      <button
                        ...
                        className="w-full flex items-center justify-between px-4 py-2.5 text-xs text-teal-700 dark:text-teal-300 hover:bg-teal-50 dark:hover:bg-teal-950/50 font-semibold transition-colors group cursor-pointer"
                      >
                        <div className="flex items-center space-x-2.5">
                          <div className="p-1 rounded-md bg-teal-100 dark:bg-teal-900/80 text-teal-600 dark:text-teal-300">
                            <SlidersHorizontal className="w-3.5 h-3.5" />
                          </div>
                          <span>Pengaturan Pengguna & AI</span>
                        </div>
                        <span className={`text-[10px] px-1.5 py-0.5 rounded font-medium ${
                          ...
                          Balon: {userSettings?.mascotSpeechBubbleEnabled ? 'ON' : 'OFF'}
                        </span>
                      </button>
                      ...
                      <button
                        id="menu-dashboard-layout-config"
                        onClick={() => {
                          setShowProfileMenu(false);
                        ...
                        title="Atur tata letak dan prioritas kartu informasi dashboard divisi"
                      >
                        <SlidersHorizontal className="w-4 h-4 text-indigo-600 dark:text-indigo-400 mr-2.5" />
                    ...
                    <div className="px-4 py-2 flex items-center justify-between text-xs hover:bg-slate-50 dark:hover:bg-slate-800/50 transition-colors">
                      ...
                      <button
                        ...
                        className="flex items-center space-x-1.5 px-2.5 py-1 text-[11px] font-semibold rounded-md bg-slate-100 dark:bg-slate-800 hover:bg-slate-200 dark:hover:bg-slate-700 text-slate-700 dark:text-slate-200 border border-slate-300 dark:border-slate-700 cursor-pointer transition-colors"
                      >
                        <span
                          className="w-2.5 h-2.5 rounded-full inline-block"
                          style={{ backgroundColor: activeColorPreset.primaryHex }}
                        />
                        <span>{activeColorPreset.name}</span>
                      </button>
                    </div>
      ...
      {/* Theme & Color Preset Customizer Modal */}
      <ThemePresetModal
        isOpen={showThemePresetModal}
        onClose={() => setShowThemePresetModal(false)}
      />

      {/* User Settings & Mascot Bubble Preferences Modal */}
      <UserSettingsModal
              ...
              {isAuthenticated ? (
                <div className="p-3.5 rounded-2xl bg-slate-50 dark:bg-slate-900/90 border border-slate-200 dark:border-slate-800 shadow-xs">
                  ...
                  <div className="grid grid-cols-2 gap-2 mt-3 pt-3 border-t border-slate-200 dark:border-slate-800">
                    ...
                    <button
                      id="btn-mobile-user-settings"
                      onClick={() => {
                        setShowMobileMenu(false);
                        setShowUserSettingsModal(true);
                      }}
                      className="flex items-center justify-center space-x-1.5 px-2.5 py-1.5 rounded-lg bg-teal-50 dark:bg-teal-950/40 border border-teal-200 dark:border-teal-800 text-teal-700 dark:text-teal-300 text-xs font-medium hover:bg-teal-100 dark:hover:bg-teal-900/60 transition-colors cursor-pointer"
                      title="Pengaturan Balon Kata & Maskot AI"
                    >
                      <SlidersHorizontal className="w-3.5 h-3.5 text-teal-600 dark:text-teal-400 shrink-0" />
              ...
              <div className="p-3.5 rounded-2xl bg-slate-50 dark:bg-slate-900/90 border border-slate-200 dark:border-slate-800 space-y-3">
                ...
                <div className="flex items-center justify-between text-xs pt-1 border-t border-slate-200 dark:border-slate-800">
                  ...
                  <button
                    ...
                    className="flex items-center space-x-1.5 px-2.5 py-1 text-[11px] font-semibold rounded-md bg-white dark:bg-slate-800 hover:bg-slate-100 dark:hover:bg-slate-700 text-slate-700 dark:text-slate-200 border border-slate-300 dark:border-slate-700 cursor-pointer transition-colors shadow-xs"
                  >
                    <span
                      className="w-2.5 h-2.5 rounded-full inline-block"
                      style={{ backgroundColor: activeColorPreset.primaryHex }}
