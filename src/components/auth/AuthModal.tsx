...
export const AuthModal: React.FC<AuthModalProps> = ({
  ...
  const [mode, setMode] = useState<'login' | 'register'>(initialMode || authModalMode || 'register');
    ...
    >
      ...
      >
        ...
        <div className="p-6 overflow-y-auto space-y-5 flex-1">
          {/* Developer / Testing Dummy Data Quick Load (Auto-hidden on publish) */}
          {SHOW_DUMMY_BUTTON && (
            <div className="p-3.5 bg-gradient-to-r from-amber-500/15 via-indigo-500/10 to-teal-500/10 border border-amber-500/30 rounded-2xl flex flex-col sm:flex-row sm:items-center justify-between gap-3 text-amber-200 shadow-sm">
              <div className="flex items-start space-x-2.5">
                ...
                <div>
                  <div className="flex items-center gap-2">
                    <span className="text-xs font-bold text-amber-300">Pintasan Uji Coba: Muat Data Dummy</span>
                    <span className="text-[9px] px-1.5 py-0.5 rounded bg-amber-400/20 text-amber-300 font-mono font-bold tracking-wider">
                      DEV ONLY
                    </span>
                  </div>
                  <p className="text-[11px] text-slate-300 mt-0.5">
                    Isi seluruh modul (katalog buku, kas, SPK, order, anggota) & otomatis masuk ke Dashboard.
                  </p>
                  <p className="text-[10px] text-amber-400/80 mt-0.5 italic">
                    *Tombol ini khusus mode pengujian & otomatis tidak akan muncul saat di-publish.
                  </p>
                </div>
              </div>

              <button
                id="btn-load-dummy-data-auth"
                type="button"
                onClick={() => {
                  loadDemoData();
                  setSuccessMsg('Data dummy lengkap berhasil dimuat! Masuk ke Dashboard sebagai Administrator...');
                  setTimeout(() => {
                    if (onClose) onClose();
                  }, 500);
                }}
                className="w-full sm:w-auto px-4 py-2 rounded-xl bg-amber-500 hover:bg-amber-400 text-slate-950 text-xs font-bold shadow-md shadow-amber-500/20 transition-all cursor-pointer flex items-center justify-center space-x-1.5 shrink-0"
              >
                <Sparkles className="w-3.5 h-3.5" />
                <span>Muat Data Dummy</span>
              </button>
            </div>
          )}
          ...
          {mode === 'login' && (
            <form onSubmit={handleLogin} className="space-y-4">
              ...
              {/* QUICK 1-CLICK DEMO LOGIN ACCOUNTS (DEV ONLY) */}
              {SHOW_DUMMY_BUTTON && (
